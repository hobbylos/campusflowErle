 ...
 *
 * Fehlender/ungueltiger Token ist ein Authentifizierungsproblem (401),
 * keine Berechtigungsfrage (403) -- daher AuthentifizierungsFehler.
 */
@Component
public class AktuellerNutzerAufloeser {
    ...
    public Nutzer aufloesen(String autorisierungsHeader) {
        ...
        return authService.nutzerFuerToken(token)
                .orElseThrow(() -> new AuthentifizierungsFehler("Nicht angemeldet oder Token abgelaufen"));
    }

    private String entferneBearerPraefix(String header) {
        if (header == null) {
            throw new AuthentifizierungsFehler("Authorization-Header fehlt");
        }
